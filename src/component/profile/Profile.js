      ...
      try {
          ...
          if (token) {
              ...
              if (postToRequest && !postToRequest.PublicRequest) {
                  ...
                  const updateResponse = await axios.put('http://localhost:5000/blog/requested', { postIds }, {
                      headers: {
                          Authorization: `Bearer ${token}`,
                      },
                  });
                                        ...
                                        <div className="artical-meta">
                                            ...
                                            <div className="actions">
                                                ...
                                                {post.PublicRequest ? (
                                                    ...
                                                    <button className="btn btn-danger" onClick={() => handleRequestPublic(post._id)}>Request Public</button>
                                                )}
                                            </div>
                                        </div>
                                        <Link to={`/blogDetail/${post._id}`} className="titles1">{post.Title}</Link> 
                                        <Link to={`/blogDetail/${post._id}`} className="article-description">{post.Content}</Link> 
                                        <Link to={`/blogDetail/${post._id}`} className="readm">Read more...</Link> 
                                    </div>
                                ))
                            ) : (
